...
integridad de datos, `ContactForm`, `ThemeToggle` y la página completa (anclas, links externos, CV).
...
- Specs: `home` (secciones, overflow mobile, anclas, links externos, 404, nav, skip link), `theme`,
  `contact` (Formspree mockeado con `page.route`), `a11y` (axe WCAG 2.1 AA en claro y oscuro),
  `seo-and-api` (meta/OG, robots/sitemap/icon/OG image, headers de seguridad, `/api/log`).
